...
typedef pair<const string, CmdExec*>  CmdRegPair;//ziyi: mandcmd(string), pointer to CmdExec
...
private:
   ...
   bool onlySpace(string&);
   void printCmd() const;
   void insertStr(string&,int len=0);
   bool strBeforeCursor(string&);
   bool spaceAfterTok(string&);
   vector<string> cmdMatchPart(string);
   string findSpecPrefix(string&);
   string findFilePrefix(vector<string>, string = "");
   string findCommonPrefix(string& ,string&,string = "");
   void findMatchFiles(vector<string>& ,vector<string>& ,string );
   void printFiles(const vector<string>&);
   string strChangeCase(string ,string );
   #ifdef TA_KB_SETTING
   void taTestOnly() {}
   #endif
