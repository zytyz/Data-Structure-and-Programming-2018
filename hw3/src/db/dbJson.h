...
public:
    ...
    bool operator !() /**not sure**/
    { 
        if(_jsonReadIn)return false;
        else return true;
    }
    // return this if JSON file has been read in; return NLL if not.
    operator void* () //const//ziyi: defines what happens when the object is converted to a void pointer
    { 
        if(_jsonReadIn)return this;
        else return NULL; 
    }
...
private:
    ...
    bool _jsonReadIn;//check if json has been read in
};
