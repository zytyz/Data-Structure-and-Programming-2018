...
initDbCmd()
{
    ...
    if (!(cmdMgr->regCmd("DBAPpend", 4, new DBAppendCmd) &&
      ...
      cerr << "Registering \"initDB\" commands fails... exiting" << endl;
      return false;
    }
    return true;
}
...
DBAppendCmd::exec(const string& option)
{
    ...
    if(!CmdExec::lexOptions(option,tokens,2))//tokens.size() should be 2
        return CMD_EXEC_ERROR;
    int num;
    if(!isValidVarName(tokens.at(0)))
    {
        cout<<"Error: Illegal option!! ("<<tokens.at(0)<<")"<<endl;
        return CMD_EXEC_ERROR;
    }
    else if(!myStr2Int(tokens.at(1),num))
    {
        cout<<"Error: Illegal option!! ("<<tokens.at(1)<<")"<<endl;
        return CMD_EXEC_ERROR;
    }
    dbjson.add(DBJsonElem(tokens.at(0),num));
...
DBReadCmd::exec(const string& option)
{
   ...
   if (options.empty())
      return CmdExec::errorOption(CMD_OPT_MISSING, "");//ziyi: missing option <filename>

   bool doReplace = false;
   string fileName;
   for (size_t i = 0, n = options.size(); i < n; ++i) 
   {
      if (myStrNCmp("-Replace", options[i], 2) == 0)//if any option in options is -r
      {
   ...
   if (dbjson) //a file is already stored in dbjson
   {
      if (!doReplace) {
