...
private:
   ...
   static int  _strLen;//maximum string length for AdtTestObj
};

class AdtTest//an object with a private member _container, which is an object of list/array
{
...
public:
   ...
   void deleteRandom(size_t repeat = 1) {
      ...
      for (size_t i = 0; i < repeat; ++i) {
         ...
         //cout<<"size of str: "<< (getPos(pos)._node->_data)._str.size() <<endl;
         //cout<< (getPos(pos)._node->_data) <<endl;
         #endif
         if (_container.erase(getPos(pos))) --s;
         //_container.pop_front(); --s;
      }
   }
...
private:
   ...
   void printForward() const {
      ...
      #ifdef ZIYI
      cout<<"tree size: "<<_container.size()<<endl;
      #endif
   }
   void printBackward() const {
      ...
      size_t r = (idx + 1) % N;
      AdtType<AdtTestObj>::iterator li = _container.end();
      //cout<<"here"<<endl;
      --li;
